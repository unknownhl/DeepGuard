...
from cosec.data_proceess import LoRA_Dataset
...
def main():
    ...
    args = parse_args()
    args.output_dir = os.path.join(args.output_dir, args.base_model + '-cosec')
    set_logging(args, os.path.join(args.output_dir, 'train.log'))
    set_devices(args)
    set_seed(args.seed)

    if args.base_model in MODEL_DIRS:
        args.base_model = MODEL_DIRS[args.base_model]

    tokenizer = AutoTokenizer.from_pretrained(args.base_model, cache_dir=cache_dir, local_files_only=True)
    if tokenizer.eos_token_id is None:
        tokenizer.eos_token_id = tokenizer.bos_token_id
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token_id = tokenizer.eos_token_id

    model = AutoModelForCausalLM.from_pretrained(args.base_model, device_map='auto', cache_dir=cache_dir, local_files_only=True)

    ref_model = AutoModelForCausalLM.from_pretrained(args.base_model, device_map='auto', cache_dir=cache_dir, local_files_only=True)
    ...
    if 'llama' in model.config.model_type.lower():
        ...
        assert 'model_type' in model.config, f'Unknown model type: {model.config.model_type}'
    ...
    train_dataset = LoRA_Dataset(args, tokenizer, mode = 'train')
    val_dataset = LoRA_Dataset(args, tokenizer, mode = 'val')
