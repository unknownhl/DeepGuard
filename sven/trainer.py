...
class PrefixTrainer(TrainerBase):
    ...
    def step(self, batch):
        ...
        inputs, weights, control_ids = batch
        ...
        if self.args.contrastive_loss_ratio != 0 or self.args.kl_loss_ratio != 0:
            ...
            if self.args.kl_loss_ratio != 0:
                ...
                corr_loss = token_weighted_loss('kl', correct_log_probs, ref_log_probs, 1-shift_weights)
                # if torch.isnan(corr_loss).any():
                #     corr_loss = torch.nan_to_num(corr_loss, nan=0.0)
                kl_loss += corr_loss
                incorrect_log_probs = F.log_softmax(incorrect_logits, dim=-1)
                incorr_loss = token_weighted_loss('kl', incorrect_log_probs, ref_log_probs, 1-shift_weights)
                # if torch.isnan(incorr_loss).any():
                #     incorr_loss = torch.nan_to_num(incorr_loss, nan=0.0)
                kl_loss += incorr_loss
