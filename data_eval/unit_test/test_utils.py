        ...
        for module_name, status, score, *exception in cls.results:
            ...
            if exception:
                ...
                if stat is not None:
                    ...
                    elif module_name + '.c' in stat:
                        stat[module_name + '.c']['functional'] = False
                    else:
                        # Initialize the entry if it doesn't exist
                        # Determine the extension based on what files exist
                        if os.path.exists(os.path.join(cls.base_file_path, module_name + '.py')):
                            stat[module_name + '.py'] = {'functional': False}
                        else:
                            stat[module_name + '.c'] = {'functional': False}
            else:
...
def test_all(path, test):
    ...
    # Load existing stat file or create empty dict
    if os.path.exists(stat_path):
        with open(stat_path, 'r') as f:
            stat = json.load(f)
    else:
